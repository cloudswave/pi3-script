...
function musicInfo(list, index) {
    ...
    if(list == rem.playlist && index == rem.playid) {   // 当前正在播放这首歌，那么还可以顺便获取一下时长。。
        var curDuration = storage.getItem("curPlayer").indexOf("media_player.") >= 0 ? storage.getItem("curMedia_duration") : rem.audio[0].duration;
        tempStr += '<br><span class="info-title">时长：</span>' + formatTime(curDuration);
    }
